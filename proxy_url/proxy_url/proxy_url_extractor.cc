...
//#else
	...
	const char* curchar=token.getCurReadPos();
	while(*curchar=='?'){
		token.skipTo('?');
		token.next();
		curchar=token.getCurReadPos();	
	} //find the first char that not be '?'
 
        std::string key;
        while (!token.isEnd()) {
            key = token.nextString('=');
	    
	    std::string::iterator beg=key.begin();
	    for( ;beg!=key.end();++beg){
	    	if(*beg=='&') break;	
	    }
	    if(beg!=key.end()){
		...
		const char* after_sub_curpos=token.getCurReadPos();
		/*This case is:raw_url="http://www.microsofttranslator.com/bv.aspx?from=&to=zh-chs&a=&xx=yy"
		 * sub_url=""
		*/

                if (sub_url.empty() && nreadable > 0 && curpos==after_sub_curpos) {
                    /**
                    * case 2: 
