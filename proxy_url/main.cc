...
void test_ProxUrlExtractor_Extract3()
{
    ...
    std::string test_data[][2] = {
        // url                                                                                          suburl,
        {"http://www.microsofttranslator.com/bv.aspx???&from=&to=zh-chs&xxx&&&&&&&&", ""},
        {"http://www.microsofttranslator.com/bv.aspx???&from=&to=zh-chs&xxx&query", ""},
        {"http://www.microsofttranslator.com/bv.aspx??from=&to=zh-chs&xxx&query", ""},
        {"http://www.microsofttranslator.com/bv.aspx?from=&to=zh-chs&xxx&query", ""},
        {"http://www.microsofttranslator.com/bv.aspx?from=&to=zh-chs&=xxx&query", ""},
    ...
    for (size_t i = 0; i < H_ARRAY_SIZE(test_data); i++)
    {
        ProxyURLExtractor::Extract(keys, test_data[i][0]);
        std::string s = ProxyURLExtractor::Extract(keys, test_data[i][0]);
        if (s != test_data[i][1]) {
            all_test_ok = false;
        }
    }
